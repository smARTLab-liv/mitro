...
void twist_cb(const geometry_msgs::Twist::ConstPtr& msg) {
    if (msg->linear.x != 0) {
        ...
        //float weight = (twist_yaw - force_yaw)
...
std::vector<float> point_to_vector(float x, float y) {
    ...
    if (dist <= MAX_RANGE) {
        ...
        float max_el = std::max(x, y);
        float weight = 1 - pow((dist - BASE_RADIUS - CLEARING_DIST), 2) / pow((MAX_RANGE - BASE_RADIUS - CLEARING_DIST), 2);
        point_vector[0] = (-x / max_el) * weight;
        point_vector[1] = (-y / max_el) * weight;
    }
    return point_vector;
}
