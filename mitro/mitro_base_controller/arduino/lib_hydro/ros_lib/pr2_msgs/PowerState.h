  ...
  class PowerState : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 3) = (this->time_remaining.nsec >> (8 * 3)) & 0xFF;
      ...
      uint32_t * length_prediction_method = (uint32_t *)(outbuffer + offset);
      *length_prediction_method = strlen( (const char*) this->prediction_method);
      offset += 4;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      *val_power_consumption |= ((uint32_t)(*(inbuffer + offset)) & 0x0f)<<19;
      ...
      offset += sizeof(this->time_remaining.nsec);
      uint32_t length_prediction_method = *(uint32_t *)(inbuffer + offset);
      offset += 4;
