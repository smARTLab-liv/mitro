...
std::vector<Point2D> twist2poly(float lin, float ang) {
    ...
    points_temp.insert( points_temp.begin(), 
                        proj(x - r * sin(-(M_PI-th)), 
                        y + r * cos(-(M_PI-th)), 0));
...
int main(int argc, char**argv)
{
    ...
    while (ros::ok()) {
        int t = new_view;
        // check if view change triggers resolution change (0 to 1, and 2 to 3 are save)
        if (t > -1 && t < 4 && t != view) { 
            if ( t < 2 && view >= 2 ) {
